...
import { getDeviceId, setTokens, getTokens } from '@/utils/api'; // Using standard relative paths if @ fails: '../utils/api'
...
export default function Login() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (refreshToken) {
        try {
          const res = await fetch('https://api.echocart.in/api/auth/login/refresh', {
            method: 'POST',
            headers: {
          ...
          if (data.token) {
            ...
            router.push('/'); // Already logged in, go to main app!
          }
        } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-black text-white p-6 flex flex-col justify-center items-center">
      <div className="w-full max-w-md bg-zinc-900 p-8 rounded-xl border border-zinc-800 shadow-2xl">
        ...
        {errorMsg && <div className="mb-4 p-3 bg-red-900/50 border border-red-500 text-red-200 rounded">{errorMsg}</div>}
